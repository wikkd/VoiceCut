...
| tests/test_speakers.py | 稳健嵌入聚合(截尾均值) / 自适应聚类(轮廓系数) / 相似簇合并 / 记忆强绑定 / label_embeddings / 嵌入编解码 / match_labels_to_pool（同声纹合并、异声纹新建、阈值边界、标签复用） |
...
## 10. 说话人音色归类增强（自适应聚类 + 角色池记忆）

**问题**：多音色归类不完整——同一人被拆成多个角色、声音相近的两人被并成一个；且每次「识别说话人」都从零聚类，只在事后用固定余弦阈值（0.82）与角色池做一次弱匹配，导致跨素材/跨次识别容易产生重复角色。

**改动（`app/speakers.py`）**：
- **稳健嵌入聚合** `_aggregate_subtitle_embedding`：字幕级声纹不再对窗口做简单平均，而是先求主体方向、剔除与主体最不一致的 25% 窗口再做截尾平均 + L2 归一化。单个被 BGM/叠话污染的窗口不再把整条字幕的声纹拉偏（均匀窗口时退化为原均值，行为兼容）。
- **自适应聚类** `_cluster_labels`：对层次树扫描 0.04~0.94 的切割阈值，用**轮廓系数**（`_silhouette_score`，样本到本簇质心 vs 到最近异簇质心的余弦距离）选择分离度最优的分区，替代固定「余弦距离 0.60」。允许 k≥2（`min_k` 只作回退参考），因为把合法的 2 人项目强行拆开会更糟；无候选时回退到原有「收紧/放松」自适应逻辑。
- **相似簇合并** `_merge_similar_clusters`：合并质心余弦 ≥ **0.95** 的簇，仅用于修补同一人因噪声被拆簇（过分割）。阈值取高有实测依据（见下）。
- **记忆强绑定** `match_labels_strong`：识别出的标签先与角色池代表声纹做强匹配——余弦 ≥ 0.80 **且领先次优 ≥ 0.05**（避免「池里最不差的那个」被强行命中）时直接归入该角色，并更新代表声纹（运行平均 + `emb_count`）；剩余标签再走原有弱匹配 `match_labels_to_pool`（≥0.82）或新建。
- **可插拔声纹模型**：`_load_embedder` 按优先级（ERES2Net > ECAPA）**只用已缓存模型**加载，避免离线冷启动在下载重试上白等 ~90s（实测从 90s 降到 2.3s）；`VC_SPEAKER_MODEL=org/repo` 可显式钉住并允许下载；实际模型名经 `embedder_name()` 上报为 `quality`。worker 侧判定从 `quality == "ecapa"` 放宽为 `quality != "mfcc"`，使更强模型同样启用池归并。

**阈值标定（真实 ECAPA 实测）**：合成 5 个音色各 6 条 1s 字幕，测两两余弦——**同类内一致性 0.93~0.98，异类相似度 0.83~0.97**，区间高度压缩。因此：合并阈值必须 > 异类相似度（取 0.95），且不能依赖绝对阈值分割，必须用相对分离度（轮廓系数）——这正是本次改动的核心依据。

**端到端验证（真实 ECAPA，非 mock）**：3 个音色（110Hz 男低 / 220Hz 女高 / 150Hz 中男）× 2 个素材 → 正确分出 3 个说话人且跨素材标签一致（A/B/C 在素材 1、2 分别同为说话人1/2/3）；第二轮新素材重识别 → 强绑定 5/5 命中已有角色、0 新建、无重复角色。

**自动化**：pytest 121/121（`test_speakers.py` 34 例，新增 9 例：截尾均值抗污染、相似(0.8)说话人可分开、相似簇合并、默认合并阈值保守(0.92 不误并)、压缩相似度区间仍能分出结构、记忆复用不新建、需唯一胜者、低相似不绑定）；ruff 全绿。

**待人工回归（真实素材）**
- 同一项目多个视频 → 「识别说话人」→ 角色池中同一角色跨素材只有一个、标签一致；声音相近的两个角色没有被并成一个。
- 用角色池已有角色的项目再点一次「识别说话人」→ 不产生重复角色（记忆复用），`emb_count` 增长。
- 如有人工纠正过的角色（改名/改色/合并/训练过）→ 重识别后不应被记忆归并破坏（`stale_characters` 只清理自动命名且未训练的残留角色）。


## 11. URL 导入改为「导入即后台下载」

**背景**：旧流程 `/api/url/open` 在 **HTTP 请求线程里**逐条 yt-dlp 解析（批量导入会阻塞），并且页面视频预览走**远端直链 Range 代理**（签名约 2 小时过期，长会话失效）。

**改动**：
- `app/bilibili.py`：`create_job` 拆成 `register_job`（只登记，不解析）+ `resolve_job`（后台解析），`create_job` 保留为兼容包装；新增 `download_video()`（外部下载器 → 单文件直链 → yt-dlp CLI 三级回退）、`_download_with_ytdlp()`、`_download_external()`、`_cleanup_ytdlp_leftovers()`、`detect_external_downloader()`、`bilibili_cookie()/sessdata()`；`download()` 支持带 cookie 与自定义进度文案。
- `app/web/bilibili.py`：`_open_url` 只登记 + 提交后台任务并**立即返回**；`_bilibili_worker` 重写为「解析 → 下载音频 → 提取 WAV → 注册素材（快）→ 下载完整视频 → 转封装本地预览 MP4」，视频环节失败只记 warning（`_attach_local_video`），音频素材不受影响。
- 前端 `doUrlOpen` 不再自动挂代理流播放，改为等后台任务完成后用**本地预览**；弹窗文案与按钮改为「后台下载」。
- `/api/config` 新增 `downloader` 字段（当前引擎 / 外部下载器路径 / cookie 状态）。

**修掉的一个真实缺陷（重要）**：B 站只提供 DASH 分离流（**无单文件 MP4**），视频必须由 yt-dlp 合并视音频。实测个别视频的视频 CDN 节点不可达而音频正常，yt-dlp 非零退出后只留下**音频分片**；旧写法只判断「目录里有没有文件」，会把这个音频分片当成视频 → 生成一个**只有声音的“预览”**。现改为：非零退出即报错 + 只接受确含视频轨的容器（`_has_video_stream` 探测）+ 清理分片残留。

**真实端到端验证（非 mock，真实网络）**：
- 导入接口 **0.001s 立即返回**（解析/下载全在后台）。
- `BV1GJ411x7h7`：55s 完成 → 音频素材 212.3s（`/api/audio` 200）+ 本地预览 16.0 MB（`/api/video` 200，`video_url` 为本地地址而非代理）。
- `BV1xx411c7mD`（该视频 CDN 不可达）：音频素材正常导入（198.9 MB WAV），视频环节降级为 warning、**不再伪造预览**，缓存无分片残留。

**自动化**：`tests/test_bilibili_import.py` 15 例（cookie/环境变量解析、外部下载器探测与参数、`register_job` 不触发解析、`resolve_job` 填充流、直链优先、外部失败回退、**音频分片不被当成视频**、非视频轨拒绝、真实视频接受并清理分片、端到端导入成功、视频失败不整体失败、并发登记线程安全、`/api/config` 暴露下载器）；pytest 136/136，ruff 全绿。

**待人工回归（真实素材）**
- 真实 B 站链接批量导入（多行）→ 接口立即返回，任务队列逐个下载；先出音频素材可立即剪辑，稍后视频预览变为本地。
- 断网 / 某个视频 CDN 不可达 → 该素材仍以「仅音频」导入成功，不产生假预览。
- 若已安装 BBDown/yutto 或配置 `VC_SESSDATA` → `/api/config` 的 `downloader.engine` 变化，画质提升。
